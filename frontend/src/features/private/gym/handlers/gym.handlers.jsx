...
// Constants
import beertionaryAdminConstants from "../constants/gym.constants";
import constants from "./constants/gym.handlers.constants";
// Libs
...
const submitHandlerCreate = async ({
  ...
  try {
    ...
    setOpenDialog(false);
    setData((prev) => [...prev, data.service]);
...
const submitHandlerEdit = async ({
  ...
  try {
    ...
    setOpenDialog(false);
    setData((prev) =>
      prev.map((item) => (item._id === selectedRow._id ? data.service : item))
    );
    toast.success(data.message);
  } catch (error) {
    if (error.response.data.errors) {
